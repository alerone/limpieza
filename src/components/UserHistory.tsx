                ...
                <>
                    ...
                    <div className="mt-6 overflow-y-auto bg-amber-200 p-3 xl:p-2 rounded-xl gap-1 items-center justify-center shadow-md flex flex-col">
                        {history.map((item: HistoryItem) => (
                            <div key={item.week} className="flex gap-4 bg-rose-500 text-white shadow-md rounded-xl px-4 py-1">
                                <span className="font-bold text-md xl:text-xl">{item.week}</span>
                                <span key={item.week} className="font-semibold text-md xl:text-xl border-l-3 pl-2  border-white">{item.task}</span>
                            </div>
                        ))}
                    </div>
                </>
